...
**问题：** 很多 Word 文档并未使用标准的 Heading 样式（`Heading 1`、`Heading 2`），而是通过手动设置字体大小和编号来模拟标题。此时 `para.style.name` 全是 `None` 或普通样式。
...
当没有编号时，通过第一个有效 run 的字体大小判断：
- 字号 ≥ 20pt → 一级标题
- 字号 ≥ 14pt → 二级标题
- 其他 → 普通段落

#### 注意事项
- 编号和标题之间可能有**不规则空格**（`\xa0` 等），正则需兼容
- 编号前缀（如 `2.1 `）和标题正文可能在**不同的 run** 中，要整体处理
- 优先用编号模式判断，字号判断作为兜底

### 4. 表格特殊处理

**单列表格可能是代码块：** 检查单元格内容前缀
- 以 `SQL\n` 或 `sql\n` 开头 → 转为 ````sql ... ```` 代码块
- 以 `JSON\n` 或 `json\n` 开头 → 转为 ````json ... ```` 代码块
- 以 `示例：` 开头 → 转为行内代码 `` `...` ``

**多列表格：** 正常转为 Markdown 表格，跳过全空的行。

### 5. 后处理规范

- 连续多个空行合并为一个空行
- 去掉文件首尾的空行
- 清理 run 级别的粗体/斜体/下划线格式
